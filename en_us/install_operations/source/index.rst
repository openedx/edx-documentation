.. _Installing, Configuring, and Running the Open edX Platform:

###########################################################################
Installing, Configuring, and Running the Open edX Platform: Ginkgo Release
###########################################################################

This guide provides instructions for using your own instance of the Open edX
platform and associated applications.

This document applies to the :ref:`Ginkgo <Open edX
Ginkgo Release>` release of the Open edX platform, which is the most recent
release. This document also contains instructions for installing Open edX
releases.

You can install any release of the Open edX platform. For most situations, edX
recommends that you install the most recent release. However, if your
organization or project is currently running an earlier release, or your
software requires a specific release, you may want to install an earlier
release.


.. toctree::
   :numbered:
   :maxdepth: 2

   front_matter/index
   platform_releases/index
   installation/index
   configuration/index
   insights/index
   ecommerce/index
   mobile
   feature_flags/feature_flag_index
   glossary

.. include:: ../../links/links.rst
