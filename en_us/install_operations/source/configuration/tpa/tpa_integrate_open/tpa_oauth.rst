...
#. In the ``lms.env.json`` file, change the value of ``FEATURES`` >
...
#. If necessary, make sure that the correct backend is specified.
   ...
   * If you are using Google, Facebook, LinkedIn, or Active Directory, open the
     ``lms.env.json`` file and look for the ``THIRD_PARTY_AUTH_BACKENDS`` list.
     By default, the file does not contain this list.

     If the ``lms.env.json`` file does not contain the
...
#. In the ``lms.auth.json`` file, add the client secret. To do this, create the
...
#. Leave **Client Secret** blank. Open edX sets the secret through
   ``lms.auth.json``, which is more secure.
...
.. note::
  ...
  times in the ``lms.auth.json`` file.
...
#. In ``lms.env.json``, change the value of ``FEATURES`` >
   ``ENABLE_THIRD_PARTY_AUTH`` to ``true`` (it is ``false`` by default).
...
#. Enable the python-social-auth authentication backend specific to that
   ...
   #. In the ``THIRD_PARTY_AUTH_BACKENDS`` setting in ``lms.env.json``, add the
...
#. Add the client secret to ``lms.auth.json``. To do this, create a new key
