...
(edx-platform/conf/local/eo/LC_MESSAGES). DO NOT CHECK THESE FILES IN. You
...
``?preview-lang=eo`` to the URL to turn on Esperanto as a dark language.
...
Finally, append ``?preview-lang=ar`` to the end of your URL to view your
feature in Arabic. Test to make sure that the UX is properly "flipped" to
right-to-left mode (note that new and changed strings will not yet be translated;
the point is simply to make sure all elements on the page properly "flip"
