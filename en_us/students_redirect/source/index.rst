.. _edX Learner's Guide:

###################
edX Learner's Guide
###################

.. note::
   The edX Learner's Guide has been moved to the `edX Help Center`_.

.. _edX Help Center: https://support.edx.org/hc/en-us

.. toctree::
   :hidden:

   SFD_introduction
   SFD_account
   SFD_update_acct_settings
   SFD_dashboard_profile_SectionHead
   SFD_enrolling
   SFD_self_paced
   SFD_start_course
   SFD_content_availability
   SFD_pursue_certs
   SFD_check_progress
   SFD_certificates
   SFD_credit_courses
   SFD_mobile
   SFD_video_player
   sfd_discussions/index
   sfd_discussions/keep_up
   sfd_discussions/explore_posts
   sfd_discussions/add_post
   sfd_discussions/provide_feedback
   sfd_discussions/anatomy
   completing_assignments/index
   completing_assignments/proctored_exams/index
   completing_assignments/proctored_exams/SFD_proctoring_prepare
   completing_assignments/proctored_exams/SFD_proctoring_take_exam
   completing_assignments/proctored_exams/SFD_proctoring_results
   completing_assignments/proctored_exams/SFD_proctoring_overview
   completing_assignments/proctored_exams/SFD_proctoring_technical_problems
   completing_assignments/proctored_exams/online_proctoring_rules_students
   completing_assignments/timed_exams
   completing_assignments/SFD_proctored_exams
   completing_assignments/SFD_mathformatting
   completing_assignments/interactive_multiple_choice
   completing_assignments/online_proctoring_rules_students
   SFD_ORA
   SFD_mathformatting
   SFD_google_docs
   SFD_teams
   SFD_bookmarks
   SFD_notes
   SFD_wiki
   SFD_licensing
   front_matter/index
   front_matter/read_me
   front_matter/browsers
   front_matter/preface
