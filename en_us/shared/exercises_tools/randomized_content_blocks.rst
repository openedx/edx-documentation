...
library to create a randomized assignments for students. 
...
libraries from which the content is drawn, and indicate the number and
optionally the type of problem that each student is randomly given. If you
specify more than one library from which to randomly draw components for the
randomized assignment, the content from all specified libraries is combined
...
.. note:: If you do not want to bring components in the randomized content block
   up to date with the latest version in the library, you do not need to take
   any action.
...
the settings for each component within the randomized content block in the same
way as you do for any other component in your course.
